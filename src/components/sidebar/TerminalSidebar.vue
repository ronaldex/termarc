...
import TerminalRenameModal from "../terminal/TerminalRenameModal.vue";
...
const emit = defineEmits<{
  ...
  editAgent: [projectId: string, agentId: string];
...
const contextMenuItems = computed<ContextMenuItem[]>(() =>
  contextMenu.value?.kind === "command" || contextMenu.value?.kind === "agent"
    ? [
        {
          id: "edit",
          label: contextMenu.value.kind === "agent" ? "Edit agent" : "Edit command",
        },
        {
          id: "delete",
          label: contextMenu.value.kind === "agent" ? "Delete agent" : "Delete command",
          danger: true,
...
async function openContextMenu(request: SidebarContextMenuRequest): Promise<void> {
  if (request.kind === "terminal") {
    ...
    emit("focus", {
      id: `${request.projectId}:${request.kind}:${request.commandId}`,
      kind: request.kind,
...
function selectContextMenuItem(id: string): void {
  ...
  if (request.kind === "terminal") {
    ...
    if (request.kind === "agent") emit("editAgent", request.projectId, request.commandId);
    else emit("editCommand", request.projectId, request.commandId);
  } else if (id === "delete") {
    if (request.kind === "agent") emit("deleteAgent", request.projectId, request.commandId);
    else emit("deleteCommand", request.projectId, request.commandId);
  }
}
...
useProjectTreeNavigation({
  ...
  onAction(action) {
    ...
    else if (action.type === "toggle-commands") emit("toggleCommands", action.projectId);
    else emit("toggleAgents", action.projectId);
  },
});
...
<template>
  ...
  >
    ...
    <ProjectTree
      ...
      @toggle-agents="emit('toggleAgents', $event)"
